...
(** QDGraph -> SPACE **)
module SpaceOfQDGraph (Graph : QDGRAPH) : (SPACE with type t = Graph.t and type point = Graph.point and type pointset = Graph.pointset ) = struct
     
    type t = Graph.t
    type point = Graph.point
    type pointset = Graph.pointset

    let set = Graph.get_nodes
...
(** QDGraph -> TIME **)
module TimeOfQDGraph (Graph : QDGRAPH) : (TIME with type t = Graph.t and type point = Graph.point and type pointset = Graph.pointset) = struct
