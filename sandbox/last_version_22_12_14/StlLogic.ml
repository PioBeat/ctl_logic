...
fun fs -> match fs with
  ...
  (* semantica af *)
  and sem_af_count = fun acc bad todo stref ->
    let count = ref PtP.empty in
    let smart_add_count = fun st ->
      count := PtP.add st (Model.st_cardinal (Model.st_time_next st (!stref))) (!count)
    in
    let _ = Model.st_iter smart_add_count (!bad) in
    count

  and sem_af_aux = fun acc todo count stref x ->
	...
	then (
      ...
      | Not_found -> ()
    in
    Model.st_iter pred_elaboration (Model.st_time_pred x (!stref))

  and sem_af = fun acc bad stref ->
    let todo = ref Model.st_empty in
    let count = sem_af_count acc bad todo stref in
    let _ = Model.st_iter (sem_af_aux acc todo count stref) (!acc) in
    while ((!todo) <> Model.st_empty) do
      Model.st_iter (sem_af_aux acc todo count stref) (!todo)
    done
