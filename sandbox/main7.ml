...
let imagefolder = Test7.imagefolder

let rgbimg = Test7.rgbimg
let oldalbum = ref(Test7.album)
let album = ref(!oldalbum)

let time = Test7.time
let (model,pr_env) = (Test7.model,ref Test7.pr_env)

let fsyntax_env = Test7.fsyntax_env
type mutable_env = Test7.mutable_env
let fs_env = Test7.fs_env
...
let _ =
  ...
  while !control do
    try
      ...
      let inp = Parser7.main Lexer7.token buf in
      match inp with
      | Interface7.LET (ide,fs) ->
...
let rec reload() =
  ...
  try
    let input = Parser7.main Lexer7.token lexbuf in
    match input with
     
    (* mostra le formule memorizzate *)
    | Interface7.SHOW_STORE ->
      MyLogic.print_env fs_env.env;
      reload()
	
    (* mostra i possibili futuri *)
    | Interface7.SHOW_FUTURE ->
    ...
    | Interface7.SET_SPACE (x,y) ->
      s0 := (x,y);
      reload()

    (* imposta il tempo zero *)
    | Interface7.SET_TIME t1 ->
      t0 := t1;
      draw_rgb ((!album) t1);
      reload()
    ...
    | Interface7.SEM_IDE (clr,fride,frnamelist) ->
    ...
    | Interface7.BACKTRACK (fs) ->
      let fbt = MyLogic.fsyntax_to_btformula fs_env.env (!pr_env) fs in
      let sxy = Interface7.xyimage_to_xyspace rgbimg (!s0) in
    ...
    | Interface7.SAVE_IMAGE filename ->
